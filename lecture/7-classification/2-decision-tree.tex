...
\begin{frame}{Decision-Tree}
	% REVIEW: I would move this slide in front of the example. After all, the individual components of a decision tree are explained here. This explanation is currently either too late, because good students have already understood these components, or because not as good students have failed to understand the example without this explanation.
	\vspace*{-0.6em}
...
% REVIEW: I have replaced the (cont'd) with (I) and (II) on this and the next slide, as this is the variant used mostly in the rest of the presentation. (Same replacement is done for slide 12/13 and 19/20)
...
\begin{frame}{Attribute Selection Measures}
	...
	\begin{itemize}
		...
		\item Partition data based on attribute with best score. \textit{Best score} minimize/maximize depends on measure used.
		      % REVIEW: Is it intended that there is a "minimize/maximize" in "Best score minimize/maximize depends on measure used."? I guess you meant "Best score depends on measure used.", but I am not sure.
...
\begin{frame}{Attribute Selection Measures: Information Gain (ID3) (I)}
	\begin{itemize}
		...
		\item \textbf{Expected information} (entropy) needed to classify a tuple in $D$:
		      % REVIEW: During the creation of the lectures on Data and Preprocessing, we had once briefly discussed numbering and especially examples (see slide 21). At that time we had agreed to omit this numbering completely. I have therefore deleted the numbering everywhere everywhere where I noticed it.
		      \begin{align*}
...
\begin{frame}{Example: Information Gain}
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			...
			% REVIEW: Regarding those tables and images that go over the edge of the slide, you had already informed me that you are aware of those and will fix them. But since I've looked at every slide in detail anyway, I'll just mark this case and any others I've noticed with a quick "REVIEW: PICTURE-NOT-IN-FRAME" comment. Then you can easily find affected slides via the search.
...
\begin{frame}{Gini Index (II)} % TODO: plot anpassen, sodass cumulative income nur steigt und nie faellt
	% REVIEW: The most important issue I intended to mention here, is already mentioned in the TODO above. I will therefore limit myself to suggesting the possible improvement of marking the areas between the curves and the line. After all, the Gini Index and thus also this image is exactly about this area. This should be recognizable in a Gini Index image.
	\centering
...
\begin{frame}{Classification in Large Databases}
	\begin{itemize}
		...
		\item Two scalable methods:
		      % REVIEW: I would phrase that differently, since there are certainly more than two methods.
		      \begin{enumerate}
			      \item RainForest
			      \item BOAT
		      \end{enumerate}
