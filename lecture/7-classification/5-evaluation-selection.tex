...
% REVIEW: I'm not sure if the TODO might even be related to the point I noticed. I'll just mention it anyway: I'm a bit confused here why two different confusion matrices are shown. Isn't one enough? I would choose only the version from (II), since more information is added there.
		...
		\item Random sampling: a variation of holdout.
		      \begin{itemize}
			      \item Repeat holdout $k$ times, i. e. create multiple random splits and
			            ...
			            % REVIEW: The text after the i.e. is very confusing in my opinion. Is this supposed to be a description of a single holdout iteration? If so, I would introduce this iteration in a separate bullet point(s). "Repeat Holdout k times" is enough for this one.
...
\begin{frame}{Evaluation Strategies: Cross Validation}
	...
	% REVIEW: In my opinion, cross validation is an umbrella term for a mass of methods (e.g. rolling cross validation, Monte Carlo cross validation or k-fold cross validation). For this reason, I think it is wrong to present k-fold Cross Validation as a synonym for the umbrella term Cross Validation here, even though it is certainly one of the best known variants.
	\begin{columns}[T]
...
\begin{frame}{Evaluating Classifier Accuracy: Null Hypothesis}
	\begin{itemize}
		...
		\item \textbf{Assume samples follow a $t$-distribution with $k-1$ degrees of freedom.}
		      \begin{itemize}
			      \item Here, $k = 10$.
			            % REVIEW: Even though this k = 10 was already listed in earlier versions of KDD, I have my doubts how useful this is here. I for my part was extremely confused here. I asked myself: Does k stand for something else in this case or why is something so obvious mentioned twice?
		      \end{itemize}
...
\begin{frame}{Estimating Confidence Intervals: Table for $t$-Distribution}
	\begin{columns}
		...
		\end{column}
		% REVIEW: I realize that this is probably something for upcoming semesters, but I think this graph and the t-distribution table are two things that we could definitely recreate in latex at some point.
	\end{columns}
\end{frame}
