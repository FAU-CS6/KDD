...
\begin{frame}{Supervised vs. Unsupervised Learning}
	\begin{columns}
		% REVIEW: After comparing with Luciano's version, I saw that you removed the information "Supervised Learning (Classification) - Unsupervised Learning (Clustering)" yourself, so I didn't add it back. I think the original hint that supervised learning is represented in our lecture mainly by classification and unsupervised learning by clustering was actually good thing. I'm sure you'll talk about it, but as a little reminder when you go back over the slides before the exam, I think it's actually appropriate if it's on the slide as well.
		...
		\item \textbf{Typical applications of classification:}
		      \begin{itemize}
			      ...
			      \item Web-page categorization: Which category is it?
			            % REVIEW: I would delete/adapt the last example as it is too general for me (classification simply becomes categorization and classes become categories). In every other example the categories/classes are described a lot more precise.
		      \end{itemize}
	\end{itemize}
\end{frame}
			      ...
			      \item Estimate \textbf{\color{airforceblue}accuracy} of the model:
			            \begin{itemize}
				            ...
				            \item More on evaluation metrics later in \fullref{class:evaluation}
				                  % REVIEW: Whether I try it directly in my self-built version of the slides or in the GitHub CI version, the link to the evalution keeps sending me to the beginning of the presentation instead of the section.
			            \end{itemize}
...
\begin{frame}{Classification -- A Two-step Process: 2. Model Usage}
	% REVIEW: I took the liberty of making two changes here that I wasn't 100% sure would work for you. Therefore let me add a short explanation:
	% 1) Test data => Testing data
	% This change is simply because "ing"-form was used for the data set on slide 5. I find it more consistent to choose it here as well.
	% 2) New highlighted tuple
	% The tuple is the only one of the tuples that contradicts the classification rule defined on slide 5. It should therefore also be recognized as "special" right away
	\centering
	\begin{tikzpicture}[
			>=latex,
			database/.style={
