...
\begin{frame}{Overall Framework of CHAMELEON}
	\begin{tikzpicture}[scale=0.9, node distance = 1cm, auto,font=\footnotesize,
		...
		info/.style={rectangle, draw, inner sep=5pt, text width=4cm, minimum 
		...
		\textbf{$k$-NN graph:} \\ $p$ and $q$ are connected if $q$ \\ is among 
		the top $k$ closest \\ neighbors of $p$.}};
		\node[draw, info] at (11.5,-5) {\parbox{\linewidth}{\centering 
		\textbf{Relative interconnectivity:} \\ connectivity of $C_1$ and $C_2$ 
		\\ over internal connectivity. \\[0.2cm] \textbf{Relative closeness:} 
