...
\begin{frame}{Central Tendency: Mean and Trimmed Mean}
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			...
			\begin{itemize}
				\item Order data.
				\item Discard lowest and highest $100\alpha\%$.
				\item Compute arithmetic mean.
			\end{itemize}
...
\begin{frame}{Central Tendency: Median (I)}
...
\begin{frame}{Central Tendency: Median (II)}
...
\begin{frame}{Central Tendency: Median (III)}
...
\begin{frame}{Central Tendency: Median (IV)}
	\textbf{Median for interval grouped data}
...
\begin{frame}{Central Tendency: Mode}
	\textbf{Mode}
	\begin{itemize}
...
\begin{frame}{Example of Mode, Median and Mean (I)}
	\begin{columns}[t]
		...
		\begin{column}{0.45\textwidth}
			...
			\textbf{Negatively Skewed Data Distribution}
			\begin{tikzpicture}[font=\sffamily,
					declare function={Gauss(\x,\y,\z,\u)=1/(\z*sqrt(2*pi))*exp(-((\x-\y+\u*(\x-\y)*sign(\x-\y))^2)/(2*\z^2));},
					every path/.style={
							>=latex,
...
\begin{frame}{Measuring the Dispersion of Data (II)}
	\begin{itemize}
		...
		\item \textbf{Outlier}: usually assigned to values lower than $Q_1 - 1.5 \cdot \text{IQR}$ / higher than $Q_3 + 1.5 \cdot \text{IQR}$.
	\end{itemize}
	\vspace*{1em}
...
\begin{frame}{Boxplot Analysis}
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.7, transform shape, semithick]
			...
			\draw[<->] (2,-1.4) -- (-4, -1.4)
			node[pos=0.5,below]{$\textsc{2*IQR}$};% left outer fence
			\draw[<->] (5, -0.8) -- (9.5,-0.8)
			node[midway,below]{$\textsc{1.5*IQR}$}; % right inner fence
			\draw[<->] (5,-1.4) -- (11, -1.4)
			node[pos=0.5,below]{$\textsc{2*IQR}$};% right outer fence
			%
			\node[below] at (10.5,0.7) {$\textbf{Outlier}$}; % mild outlier on the right
			\node[below] at (-3.5,0.7) {$\textbf{Outlier}$}; % extreme outlier on the left
			% Axis
			\draw (-5,-2) -- (12.1,-2);
			% Note that the snaked line is drawn to 13 to force
			% TikZ to draw the final tick.
			\draw[snake=ticks,segment length=1cm] (-5,-2) -- (13,-2);
		\end{tikzpicture}
	\end{center}
	\vspace*{0.5em}
	\begin{itemize}
		...
		\item Whiskers: two lines outside the box, reaching the minimum and the maximum (excluding outliers).
...
\begin{frame}{Histogram Analysis}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[faucyan,ybar,fill, fill opacity=0.62, bar width = 0.8,] table {data/data.dat};
